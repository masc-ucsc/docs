...
relative order between puts (`order`). If no relative order is provided, the
messages are kept to the end of the cycle, and then printed in alphabetical
order. This is done to be deterministic. The messages generated by assertions
also get serialized like puts statements.
